...
q3_normalized_P_F_given_B        = q3_nonnormalized_P_F_given_B / (q3_nonnormalized_P_F_given_B + (1 - q3_P_F) * q3_P_L)
q3_normalized_P_not_F_given_B    = q3_nonnormalized_P_not_F_given_B / (q3_nonnormalized_P_F_given_B + (1 - q3_P_F) * q3_P_L)
...
def sense(p,colors,measurement,sensor_right):
    ### start copied from lecture solution
    S = 0.0
    sensor_wrong = 1 - sensor_right
    temp = [[0.0 for col in range(len(p[0]))] for row in range(len(p))]
    for i in range(len(p)):
        for j in range(len(p[i])):
            hit = (colors[i][j] == measurement)
            temp[i][j] = p[i][j] * (hit * sensor_right + (1 - hit) * sensor_wrong)
            S += temp[i][j]
    for i in range(len(p)):
        for j in range(len(p[0])):
            temp[i][j] = temp[i][j] / S
    ### end copied from lecture solution
    return temp
...
def move(p,motion,p_move):
    ...
    for i in range(len(p)):
        for j in range(len(p[i])):
            temp[i][j] = (p_move * p[(i-motion[0]) % len(p)][(j - motion[1]) % len(p[i])]) + (p_stay * p[i][j])
    ### end copied from lecture solution
    return temp
